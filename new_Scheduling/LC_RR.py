...
class  LC_RR():
    def __init__(self, processor_num):
        ...
        self.processor = [["."] for _ in range(processor_num)]
        self.process_num = 0                                # 프로세스 수
        self.process_run = []                               # 프로세스가 진행중인가 (-1: 종료, 0: 실행안하는중, 1: 진행중)
        self.process_id = []                                # 프로세스 명
        self.burst_time = []                                # 실행시간
        self.arrival_time = []                              # 도착시간
        self.processing_time = []                           # 프로세서안에서 실행한 시간
        self.ready_queue = []                               # 레디큐 리스트
        self.current_time = 0                               # 현재시간
        self.now_process = [0] * processor_num              # 프로세서별 현재 진행중인 프로세스
        self.result = []                                    # 결과 테이블   
        self.quantum = [0] * self.processor_num             # 사용 제한 시간(퀀텀)
        self.ready_queue_progress = []

    def set_Quantum(self, quantum):
        self.time_quantum = quantum                         # 퀀텀 입력받은 값으로 설정

    def add_Process(self, id, at, bt) :
        ...
        while sum(self.process_run) != (-1) * self.process_num:                 # 프로세스 모두 종료되면 반복 종료
            ...
            for i in range(self.processor_num):                                 # 프로세서 수만큼 반복
                    if self.processor_run[i] == 0:                              # 현재 프로세서가 일하는 중이 아니고
                        ...
                        elif (self.quantum[i] == self.time_quantum):                  # 퀀텀이 입력받은 값과 같게 되면
                            if(self.burst_time[self.now_process[i]] <= int(self.time_quantum /2)+1):
                                pass
                            else:
                                if len(self.ready_queue) > 0:                           # 레디큐에 프로세스가 존재하면
                                    self.processor_run[i] = 0                           # 프로세서는 쉬는 중
                                    self.process_run[self.now_process[i]] = 0           # 프로세스 할당 해지
                                    self.ready_queue.append(self.now_process[i])        # 현재 프로세스는 다시 레디큐에 넣음
                                    job = self.ready_queue.pop(0)                       # 레디큐에서 프로세스를 뺌
                                    self.processor_run[i] = 1                           # 프로세서는 일하는중(할당함)
                                    self.process_run[job] = 1                           # 프로세스 진행중
                                    self.now_process[i] = job                           # 이 프로세스는 현재 할당된 프로세스
...
a = LC_RR(processor_num)                                 # LC_RR 객체 생성
...
# Time Quantum 설정
print("Time Quantum을 설정해주세요 : ", end="")
quantum = input()
a.set_Quantum(int(quantum))
print("---------------------------------------------------------------------------")
