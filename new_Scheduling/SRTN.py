    ...
    def SRTN_Scheduling(self):
        while sum(self.process_run) != (-1) * self.process_num:                 # 프로세스 모두 종료되면 반복 종료
            ...
            for i in range(self.processor_num):
                if self.processor_run[i] == 1:                                          # 프로세서 일 중 
            ...
            for i in range(self.processor_num):
                if len(self.ready_queue) > 0:                                   # 레디큐에 프로세스가 존재하면   
                    if self.burst_time[self.now_process[i]] > self.burst_time[self.ready_queue[0]]: # BT가 현재보다 레디큐에 있는게 더 작으면
                                                                                # 뒤에도 비어있는 프로세서가 없다면
                        self.ready_queue.append(self.now_process[i])            # 현재 프로세스는 다시 레디큐에 넣음
                        self.process_run[self.now_process[i]] = 0                           # 프로세스 실행 중지
                        job = self.ready_queue.pop(0)                           # 레디큐에서 프로세스를 뺌
                        self.process_run[job] = 1                               # 프로세스 진행중
                        self.now_process[i] = job                               # 이 프로세스는 현재 할당된 프로세스
                        self.ready_queue = self.priority(self.ready_queue)      # 재정렬
...
a = SRTN(processor_num)                                 # SRTN 객체 생성
