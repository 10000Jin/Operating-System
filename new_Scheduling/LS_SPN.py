...
class  LS_SPN():
    ...
    def LS_SPN_Scheduling(self):
        while sum(self.process_run) != (-1) * self.process_num:                 # 프로세스 모두 종료되면 반복 종료
            ...
            self.ready_queue = self.priority(self.ready_queue)

            temp = copy.deepcopy(self.ready_queue)
            self.ready_queue_progress.append(temp)

            for i in range(self.processor_num - self.processor_num // 3):                                 # 프로세서 수만큼 반복
            ...
            for i in range(self.processor_num - self.processor_num // 3 , self.processor_num):                                 # 프로세서 수만큼 반복
                if self.processor_run[i] == 0:                                  # 현재 프로세서가 일하는 중이 아니고
                    if len(self.ready_queue) > 0:                               # 레디큐에 프로세스가 존재하면
                        job = self.ready_queue.pop()                            # 레디큐에서 프로세스를 뺌
                        self.processor_run[i] = 1                               # 프로세서는 일하는 중(할당함)
                        self.process_run[job] = 1                               # 프로세스 진행중
                        self.now_process[i] = job                               # 이 프로세스는 현재 할당된 프로세스
    ...
    def priority(self, list):                                                       # 우선순위 결정 함수
        ...
        tmp_list.sort(key = lambda x: (x[2], x[1]))                                 # BT 그 다음 AT순으로 정렬
...
a = LS_SPN(processor_num)                                 # LS_SPN 객체 생성
