...
class  HRRN():
    ...
    def add_Process(self, id, at, bt) :                     # 프로세스 추가 함수
        ...
        self.result.append([0, id, at, bt])                 # 결과 테이블에 한번에 추가
            ...
            for i in range(self.processor_num):                                 # 프로세서 수만큼 반복
                    if self.processor_run[i] == 0:                              # 현재 프로세서가 일하는 중이 아니고
                        ...
                        if self.burst_time[self.now_process[i]] == 0:           # 실행시간이 0이 되면
                            ...
                            TT = self.current_time - self.result[self.now_process[i]][2]    # TT = 현재시간 - AT
                            WT = TT - self.result[self.now_process[i]][3]                   # WT = TT - BT
                            NTT = round(TT / self.result[self.now_process[i]][3], 1)        # NTT = TT / BT
    ...
    def priority(self, list):                                                       # 우선순위 결정 함수
        ...
        for i in range(len(list)):
            ...
            response_ratio = (self.current_time - self.arrival_time[a] + self.burst_time[a]) / self.burst_time[a]    # (WT + BT) / BT 
            self.result[a][0] = response_ratio                                      # 프로세서 정보의 0번지에 저장
            tmp_list.append(self.result[a])                                         # 임시 리스트에 해당 프로세서 정보 저장
        tmp_list.sort(key = lambda x: (-x[0], x[3]))                                # response ratio 높은순 그 다음 BT 낮은순으로 정렬
                    
        for i in range(len(tmp_list)):
            tmp = tmp_list.pop(0)                                                   # 임시리스트에서 프로세스 정보를 뽑아
            for j in range(self.process_num):
                if self.process_id[j] == tmp[1]:                                    # 프로세스 명이 일치하는 것이 있으면 
...
a = HRRN(processor_num)                                 # HRRN 객체 생성
